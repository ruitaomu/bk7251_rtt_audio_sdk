...
UINT32 mcu_ps_machw_init(void)
{
    ...
    machw = ((UINT64)nxmac_monotonic_counter_high_2_getf())<<32;
    machw+=(UINT64)hal_machw_time();
    mcu_ps_machw_save.machw_tm = machw;
...
UINT32 mcu_ps_machw_cal(void)
{
    ...
    tmp1 /= 1000;
    if((UINT32)tmp1  < (UINT32)tmp2)
    {
        lost = (0xFFFFFFFF - (UINT32)tmp2) + (UINT32)tmp1 ;
    }
    else
    {
        lost = (UINT32)tmp1  - (UINT32)tmp2;
    }
