...
int i2s_test_loopback()
{
    ...
    /* READ */
    {
        int offset = 0;
        index = 1;
        while (offset < I2S_TEST_BUFFER_LENGTH * sizeof(buffers[index][0]))
        {
            result = rt_device_read(i2s_device, 0, buffers[index] + offset, I2S_TEST_BUFFER_LENGTH * sizeof(buffers[index][0]) - offset);
            offset += result;
...
static int i2s_test_rtt(int argc, char *argv[])
{
    ...
    if ('b' == argv[1][0])
    {
        rt_kprintf("%s both mode, will recieve data\r\n", __FUNCTION__);
        return i2s_test_loopback();
    }
    else if ('m' == argv[1][0])
    {
        master = 1;
