...
static void ble_main( void *arg )
{
    ...
    bk_rtos_deinit_queue(&ble_msg_que);
    ble_msg_que = NULL;
#if (CFG_DEFAULT_RF_USER == CFG_RF_USER_BLE)
    rf_loop = 0;
#endif
...
static void ble_thread_start(void)
{
    ...
    {
    	ret = bk_rtos_init_queue(&ble_msg_que, 
    							"ble_msg_queue",
    							sizeof(BLE_MSG_T),
    							BLE_MSG_QUEUE_COUNT);
        ASSERT(0 == ret);
        
    	ret = bk_rtos_create_thread(&ble_thread_handle, 
    			6,
    			"ble", 
    ...
    {
        rf_loop = 1;
        bk_rtos_create_thread(&rf_switch_thread, 15, "rf_switch_loop", rf_switch_loop, 2048, &rf_loop);
    }
#endif
}
...
void ble_send_msg(UINT32 data)
{
    ...
    if(ble_msg_que) {
    	...
    	ret = bk_rtos_push_to_queue(&ble_msg_que, &msg, BEKEN_NO_WAIT);
    	if(0 != ret)
    	{
...
{
    if(!bk_rtos_is_queue_empty(&ble_msg_que))
    {
        return 0;
    }
    else
